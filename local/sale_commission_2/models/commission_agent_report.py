...
class CommissionAgentReport(models.Model):
    ...
    def _get_invoiced(self):
        ...
        for order in self:
            invoices = order.commission_line.invoice_lines.move_id.filtered(lambda r: r.move_type in ('out_invoice', 'out_refund'))
    ...
    number = fields.Char(string='Report number', required=True, copy=False, readonly=True, states={'draft': [('readonly', False)]}, index=True, default=lambda self:self.env['ir.sequence'].next_by_code('commission.agent.report'))
    ...
    incoming_number = fields.Integer(string='Report number', track_visibility='onchange')
    incoming_signature_date = fields.Date(track_visibility='onchange')
    ...
    def create_invoices(self, grouped=False, final=False):
        ...
        for move in moves:
            move.message_post_with_view('mail.message_origin_link',
                values={'self': move, 'origin': move.line_ids.mapped('commission_line_ids.report_id')},
    ...
    def _prepare_report(self, partner_id, agreement_id, commission_id, date_from, date_to):
        """
        Prepare the dict of values to create the new invoice for a sales order. This method may be
        overridden to implement custom invoice generation (making sure to call super() to establish
        a clean extension chain).
    ...
    def create_report(self, partner_id, agreement_id, commission_id, date_from, date_to):
        """
        Create the invoice associated to the SO.
        :param grouped: if True, invoices are grouped by SO id. If False, invoices are grouped by
                        (partner_invoice_id, currency)
        ...
        report_vals_list = []
        # Report values.
        report_vals = self._prepare_report(partner_id, agreement_id, commission_id, date_from, date_to)

        # Invoice line values (keep only necessary sections).
        commission_line = self._collecting_data_for_report(date_from, date_to)
        for line in commission_line:
            ...
            res = {
                ...
                'price_unit': line.retail_price_withdisc_rub,
                'display_type': False,
            }
            report_vals['commission_line'].append((0, 0, res))
...
class CommissionAgentReportLine(models.Model):
    ...
    def supplier_remuneration(self):
        supplier_remuneration = self.price_unit - self.remuneration
        return supplier_remuneration
