...
class WBDetailSalesReport(models.Model):
    ...
    sales_amount = fields.Float(string='Sales amount', store=True, readonly=True, compute='_sales_amount', tracking=4)
    amount_commission_wb = fields.Float(string='Commission WB', store=True, readonly=True, compute='_amount_commission_wb')
    amount_transfer_supplier = fields.Float(string='To transfer to the supplier', store=True, readonly=True, compute='_amount_transfer_supplier')
    ...
    logistics = fields.Float(string='Logistics', store=True, readonly=True, compute='_logistics')
    percentage_logistic = fields.Integer(string='Percentage of logistics', store=True, readonly=True, compute='_percentage_logistic')
    warehouse_storage = fields.Float(string='Warehouse storage', required=True, default=0)
    percentage_storage = fields.Integer(string='Percentage of storage', store=True, readonly=True, compute='_percentage_storage')
    received_bank_account = fields.Float(string='Received to your Bank account', required=True, default=0)
    sum_wholesale_prices = fields.Float(string='Sum of the wholesale prices', store=True, readonly=True, compute='_sum_wholesale_prices')
    ...
    amount_net_income = fields.Float(string='Amount of net income', store=True, readonly=True, compute='_amount_net_income')
    ...
    def round(self, num):
        var = int(num + (0.5 if num > 0 else -0.5))
        return var

    @api.depends('report_line.retail_price_withdisc_rub')
    def _sales_amount(self):
        for line in self.report_line:
            if line.supplier_oper_name == 'Логистика':
                continue
            self.sales_amount += line.retail_price_withdisc_rub
    ...
    def _amount_transfer_supplier(self):
        for line in self.report_line:
            if line.supplier_oper_name == 'Логистика':
                continue
            self.amount_transfer_supplier += line.for_pay
    ...
    def _amount_commission_wb(self):
        ...
        if self.amount_commission_wb == 0:
            self.total_percentage_WB = 0
        else:
            self.total_percentage_WB = round(self.amount_commission_wb / self.sales_amount * 100)

    @api.depends('report_line.delivery_rub')
    def _logistics(self):
        for line in self.report_line:
            if line.supplier_oper_name == 'Продажа':
                continue
            self.logistics += line.delivery_rub

    @api.depends('logistics', 'sales_amount')
    def _percentage_logistic(self):
        if self.logistics == 0:
            self.percentage_logistic = 0
        else:
            self.percentage_logistic = round(self.logistics / self.sales_amount * 100)

    @api.depends('warehouse_storage', 'sales_amount')
    def _percentage_storage(self):
        if self.warehouse_storage == 0:
            self.percentage_storage = 0
        else:
            self.percentage_storage = round(self.warehouse_storage / self.sales_amount * 100)

    @api.depends('report_line.standard_price')
    def _sum_wholesale_prices(self):
        for line in self.report_line:
            if line.supplier_oper_name == 'Логистика':
                continue
            self.sum_wholesale_prices = line.standard_price

    @api.depends('sum_wholesale_prices', 'sales_amount')
    def _percentage_purchases(self):
        if self.sum_wholesale_prices == 0:
            self.percentage_purchases = 0
        else:
            self.percentage_purchases = round(self.sum_wholesale_prices / self.sales_amount * 100)
    ...
    def _percentage_net_income(self):
        if self.amount_net_income == 0:
            self.percentage_net_income = 0
        else:
            self.percentage_net_income = round(self.amount_net_income / self.sales_amount * 100)
    ...
    def _total_number_sales(self):
        for line in self.report_line:
            if line.supplier_oper_name == 'Логистика':
                continue
            self.total_number_sales += line.quantity
            if line.office_name in ('Коледино', 'Склад МЛП-Подольск 5'):
                self.total_number_sales_koledino += line.quantity
            if line.office_name == 'Санкт-Петербург склад':
                self.total_number_sales_spb += line.quantity
            if line.office_name == 'Краснодар':
                self.total_number_sales_kras += line.quantity
            if line.office_name == 'Новосибирск':
                self.total_number_sales_novosib += line.quantity
            if line.office_name == 'Хабаровск':
                self.total_number_sales_khab += line.quantity
            if line.office_name == 'Екатеринбург':
                self.total_number_sales_ekb += line.quantity
            if line.office_name == 'Склад Казань':
                self.total_number_sales_kazan += line.quantity

    @api.depends('sum_wholesale_prices', 'total_number_sales')
    def _average_wholesale_price(self):
        if self.sum_wholesale_prices == 0:
            self.average_wholesale_price = 0
        else:
...
class WBDetailSalesReportLine(models.Model):
    ...
    def _net_profit(self):
        for line in self:
            line.net_profit = line.for_pay - line.standard_price
